...
echo "# Check for system updates at random interval every 0-5 days" >> "$ONIONDIR"/crontab-tmp;
echo '0  0    * * *   root   /etc/onion-node/random-unattended-upgrades.sh' >> "$ONIONDIR"/crontab-tmp;
