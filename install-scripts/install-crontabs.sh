...
echo "# Check every 30 minutes if Tor and bitcoind are running" >> /etc/node-scripts/crontab-tmp;
echo '*/30  *    * * *   root   /etc/node-scripts/check-tor-bitcoin-running.sh' >> /etc/node-scripts/crontab-tmp;
echo "#" >> /etc/node-scripts/crontab-tmp;
